"""
app.py — HydroStatic AI  v3.3
댐 수문 설계 AI 보조 정수력 계산기
수정: API 다중 지원(Gemini/OpenAI/Claude), 기존 시각화/이론 탭 100% 복구
"""
...
from llm_parser import (extract_params, explain_result, chat_response,
                         is_calculation_request, test_api_key, rule_parse)
...
.hydro-title {
    ...
    text-shadow:0 0 22px rgba(0,200,255,.5); margin:0;
}
.hydro-sub {
    font-family:'Share Tech Mono',monospace; font-size:.68rem;
    color:var(--dim); letter-spacing:.22em; margin-top:.2rem;
}

.chat-wrap { max-height:60vh; overflow-y:auto; padding:.4rem 0;
    scrollbar-width:thin; scrollbar-color:var(--border) transparent; }
.chat-wrap::-webkit-scrollbar{width:4px}
.chat-wrap::-webkit-scrollbar-thumb{background:var(--border);border-radius:2px}

.msg-user { display:flex; justify-content:flex-end; margin:.45rem 0; animation:fadeR .25s ease; }
.msg-ai   { display:flex; justify-content:flex-start; margin:.45rem 0; animation:fadeL .25s ease; }
...
.bubble-user {
    ...
    border:1px solid rgba(0,200,255,.3); border-radius:12px 12px 2px 12px;
    padding:.65rem 1rem; max-width:72%;
    font-family:'Exo 2',sans-serif; font-size:.87rem; color:#e0f4ff;
    box-shadow:var(--glow);
}
.bubble-ai {
    background:var(--card); border:1px solid var(--border);
    border-left:3px solid var(--cyan); border-radius:2px 12px 12px 12px;
    padding:.7rem 1rem; max-width:88%;
    font-family:'Exo 2',sans-serif; font-size:.86rem; color:var(--text); line-height:1.65;
}
.bubble-ai code { background:#0a2033; color:var(--teal);
    font-family:'Share Tech Mono',monospace; font-size:.8rem; padding:1px 5px; border-radius:3px; }
.msg-meta { font-family:'Share Tech Mono',monospace; font-size:.61rem;
    color:var(--dim); margin:.1rem .25rem; }
.avatar-ai { width:26px; height:26px; background:linear-gradient(135deg,var(--panel),#0a2033);
    border:1px solid var(--cyan); border-radius:50%;
    display:flex; align-items:center; justify-content:center;
    font-size:.75rem; margin-right:.45rem; flex-shrink:0;
    box-shadow:0 0 8px rgba(0,200,255,.18); }

.result-card {
    background:var(--card); border:1px solid var(--border);
    border-top:2px solid var(--teal); border-radius:2px; padding:.9rem 1.1rem; margin:.5rem 0;
}
.result-title {
    ...
    margin-bottom:.55rem; padding-bottom:.3rem; border-bottom:1px solid var(--border);
}
...
.metric-label { font-family:'Share Tech Mono',monospace; font-size:.6rem;
    color:var(--dim); letter-spacing:.08em; text-transform:uppercase; }
.metric-value { font-family:'Rajdhani',sans-serif; font-size:1.1rem;
    font-weight:600; color:var(--cyan); margin-top:1px; }
.metric-sym   { font-family:'Share Tech Mono',monospace; font-size:.68rem;
    color:var(--teal); }
.metric-unit  { font-size:.68rem; color:var(--dim); margin-left:2px; }

.formula-box {
    background:#050f1a; border:1px solid rgba(0,200,255,.18);
    border-left:3px solid var(--teal); border-radius:2px;
    padding:.55rem .85rem; font-family:'Share Tech Mono',monospace;
    font-size:.78rem; color:var(--teal); margin:.35rem 0; line-height:1.9;
}

.api-ok   { background:rgba(0,255,159,.06); border:1px solid rgba(0,255,159,.3);
    border-radius:2px; padding:.45rem .7rem; font-size:.78rem; color:#00ff9f; }
.api-fail { background:rgba(255,107,53,.06); border:1px solid rgba(255,107,53,.3);
    border-radius:2px; padding:.45rem .7rem; font-size:.78rem; color:#ff9a7a; }
.api-none { background:rgba(90,122,138,.06); border:1px solid rgba(90,122,138,.3);
    border-radius:2px; padding:.45rem .7rem; font-size:.78rem; color:var(--dim); }
...
.stTextArea>div>div>textarea {
    ...
    font-family:'Exo 2',sans-serif !important; font-size:.87rem !important;
}
...
[data-testid="stSidebar"] .stMarkdown p {
    color:var(--text) !important; font-family:'Exo 2',sans-serif !important; font-size:.82rem !important;
}
...
.info-box {
    ...
    font-family:'Exo 2',sans-serif; font-size:.81rem; color:var(--dim); margin:.35rem 0;
}
</style>
...
def _rule_parse(text: str):
    """규칙 기반 파서 (llm_parser.rule_parse 위임)"""
    return rule_parse(text)

def _fmt_result(r):
    lines = [
        f"✅ **{r['shape']} 정수력 계산 완료** *(규칙 기반 파싱)*\n",
        "| 항목 | 값 | 단위 |",
        "|------|-----|------|",
        f"| 면적 A | {r['A']:.4f} | m² |",
        f"| 도심 깊이 h_c | {r['h_c']:.4f} | m |",
        f"| **정수력 F** | **{r['F']/1000:.3f}** | **kN** |",
        f"| 압력중심 y_cp | {r['y_cp']:.4f} | m |",
        f"| 편심거리 e | {r['y_cp']-r['h_c']:.5f} | m |",
        f"| 전도 모멘트 M | {r.get('M_overturn',0)/1000:.3f} | kN·m |",
    ]
    if r['shape_en']=='parabola':
        lines += [
            f"| 수평 성분 Fh | {r['F_horizontal']/1000:.3f} | kN |",
            f"| 수직 성분 Fv | {r['F_vertical']/1000:.3f} | kN |",
            f"| 합력 방향 θ  | {r.get('angle_deg',0):.1f} | ° |",
        ]
    lines += ["","> 📊 **[3D 시각화 탭]**에서 압력 분포 및 합력 벡터를 확인하세요."]
    return "\n".join(lines)
...
with st.sidebar:
    ...
    <div style="text-align:center;padding:.7rem 0 .4rem">
        <div style="font-family:'Rajdhani',sans-serif;font-size:1.25rem;
                    font-weight:700;color:#00c8ff;letter-spacing:.15em;">HYDROSTATIC AI</div>
        <div style="font-family:'Share Tech Mono',monospace;font-size:.58rem;
                    color:#5a7a8a;letter-spacing:.18em;margin-top:2px;">DAM GATE DESIGN v3.3</div>
    ...
    st.markdown("<div style='font-size:0.75rem; color:#00ff9f; margin-bottom:10px; margin-top:-5px;'>💡 잔액 부족 오류가 뜬다면 무료인 Gemini를 선택해 보세요!</div>", unsafe_allow_html=True)

    api_input = st.text_input("API Key 입력", value=st.session_state.api_key,
                               type="password", placeholder="여기에 API 키를 붙여넣으세요")
    ...
    if status == 'ok':
        st.markdown(f"""<div class="api-ok">✅ API 정상 작동<br>
            <span style="font-size:.7rem;opacity:.8">모델: {st.session_state.api_model}<br>{st.session_state.api_err_msg}</span></div>""",
            unsafe_allow_html=True)
    elif status == 'fail':
        st.markdown(f"""<div class="api-fail">❌ API 오류<br>
            <span style="font-size:.7rem;opacity:.8">{st.session_state.api_err_msg}</span></div>""",
            unsafe_allow_html=True)
    else:
        st.markdown('<div class="api-none">⬜ API 키 미검증 — 위 버튼으로 확인</div>',
                    unsafe_allow_html=True)
    ...
    with st.expander("🔑 API 키 무료 발급 방법 보기"):
        ...
        • [Create new secret key] 클릭<br>
        • <i>주의: 계정에 크레딧($5 이상) 충전 필요</i><br><br>
        ...
        • [Create Key] 클릭<br>
        • <i>주의: 계정에 크레딧($5 이상) 충전 필요</i>
        </div>
        """, unsafe_allow_html=True)

    st.markdown("---")
    st.markdown("### 📐 형상 선택")
    sel_shape = st.selectbox("수문 형상", list(SHAPE_KR.keys()),
                              format_func=lambda x: f"{SHAPE_KR[x]}  ({x})")
    ...
    params['h_w'] = st.slider("수위 h_w [m]", 0.0, 20.0, hw_def, 0.1,
                               help="수면→수문 상단(원형은 도심)까지 거리")

    st.markdown("---")
    calc_btn = st.button("▶ 정수력 계산 실행", use_container_width=True)

    st.markdown(f"""<div style="font-family:'Share Tech Mono',monospace;font-size:.6rem;
        color:#3a5a6a;text-align:center;padding:.4rem 0;">
        γ = 9,810 N/m³  |  SI 단위계<br>SymPy · SciPy · Multi-LLM API<br>Provider: {st.session_state.api_provider.upper()}
    </div>""", unsafe_allow_html=True)
  ...
  <div style="display:flex;align-items:center;gap:.8rem;">
    ...
    <div>
      <div class="hydro-title">HYDROSTATIC  AI
        <span style="font-size:.65rem;background:rgba(0,200,255,.1);border:1px solid rgba(0,200,255,.3);
              color:#00c8ff;padding:2px 7px;border-radius:2px;letter-spacing:.12em;margin-left:.5rem;">v3.3</span>
      </div>
    ...
    with col_chat:
        ...
        if not st.session_state.messages:
            ...
            <div style="text-align:center;padding:2.2rem 1rem;color:#3a5a6a;">
                ...
                <div style="font-family:'Rajdhani',sans-serif;font-size:1rem;color:#00c8ff;
                            letter-spacing:.1em;margin-bottom:.35rem;">HYDROSTATIC AI 준비완료</div>
        ...
        for i,(lbl,txt) in enumerate(examples):
            with ecols[i%3]:
                if st.button(f"▸ {lbl}", key=f"ex{i}", use_container_width=True):
                    # 예시 버튼 → 바로 메시지로 처리 (pending이 아니라 즉시 실행)
        ...
        with st.form('chat_form', clear_on_submit=True):
            user_input = st.text_input(
                "입력", value='',
                placeholder="수문 조건 또는 질문을 입력하세요... (Enter 또는 전송 버튼)",
        ...
        # ── 실제 처리할 메시지 결정 ──────────────────
        # 예시 버튼 클릭 → pending_msg / 폼 전송 → user_input
        msg_txt = ''
        ...
        if msg_txt:
            ...
            with st.spinner("🔄 분석 및 계산 중..."):
                if is_calc:
                    # ── 계산 요청: API 시도 → 실패 시 규칙 기반 ──
                    llm_ok = False
                    explanation = ""

                    if api_key:
                        try:
                            extracted, interp = extract_params(msg_txt, provider, api_key)
                            if extracted and 'shape' in extracted and 'params' in extracted:
                                result = _run_calc(extracted['shape'], extracted['params'])
                                llm_ok = True
                                try:
                                ...
                                if explanation:
                                    reply += f"---\n\n{explanation}\n\n---\n"
                                reply += "> 📊 **[3D 시각화 탭]**에서 시각화를 확인하세요."
                        except Exception:
                            llm_ok = False

                    if not llm_ok:
                        # 규칙 기반 fallback (API 없거나 실패)
                        parsed = _rule_parse(msg_txt)
                        try:
                            result = _run_calc(parsed['shape'], parsed['params'])
                            # 로컬 해설 생성
                            from llm_parser import _local_explain
                            explanation = _local_explain(result)
                            if not api_key:
                                note = "ℹ️ *API 미연결 — 규칙 기반 계산*"
                            else:
                                note = "⚠️ *API 응답 실패 → 규칙 기반 계산*"
                            reply = (f"{note}\n\n"
                                     f"✅ **{result['shape']} 정수력 계산 완료**\n\n"
                                     f"---\n\n{explanation}\n\n---\n"
        ...
        if r:
            ...
            <div class="result-card">
                ...
                <div class="metric-grid">
                    <div class="metric-item">
                        <div class="metric-label">정수력 F</div>
                        <div class="metric-value">{r['F']/1000:.3f}<span class="metric-unit">kN</span></div>
                    </div>
                    <div class="metric-item">
                        <div class="metric-label">도심 깊이 h<sub>c</sub></div>
                        <div class="metric-value">{r['h_c']:.3f}<span class="metric-unit">m</span></div>
                    </div>
                    <div class="metric-item">
                        <div class="metric-label">압력중심 y<sub>cp</sub></div>
                        <div class="metric-value">{r['y_cp']:.3f}<span class="metric-unit">m</span></div>
                    </div>
                    <div class="metric-item">
                        <div class="metric-label">면적 A</div>
                        <div class="metric-value">{r['A']:.3f}<span class="metric-unit">m²</span></div>
                    </div>
                    <div class="metric-item">
                        <div class="metric-label">편심거리 e</div>
                        <div class="metric-value">{r['y_cp']-r['h_c']:.4f}<span class="metric-unit">m</span></div>
                    </div>
                    <div class="metric-item">
                        <div class="metric-label">I<sub>G</sub></div>
                        <div class="metric-value">{r.get('IG',0):.4f}<span class="metric-unit">m⁴</span></div>
                    </div>
                    <div class="metric-item">
                        <div class="metric-label">전도 모멘트 M</div>
                        <div class="metric-value">{r.get('M_overturn',0)/1000:.3f}<span class="metric-unit">kN·m</span></div>
                    </div>
                </div>
            </div>""", unsafe_allow_html=True)
            ...
            if r['shape_en'] == 'parabola':
                ...
                <div class="result-card">
                    ...
                    <div class="metric-grid">
                        <div class="metric-item">
                            <div class="metric-label">수평 F<sub>h</sub></div>
                            <div class="metric-value">{r['F_horizontal']/1000:.3f}<span class="metric-unit">kN</span></div>
                        </div>
                        <div class="metric-item">
                            <div class="metric-label">수직 F<sub>v</sub></div>
                            <div class="metric-value">{r['F_vertical']/1000:.3f}<span class="metric-unit">kN</span></div>
                        </div>
                    </div>
                </div>""", unsafe_allow_html=True)

            # SymPy 기호 공식 — LaTeX로 렌더링
            st.markdown("---")
...
with tab_calc:
    ...
    if r:
        ...
        with dc:
            ...
            st.markdown(f"""
            <div class="result-card">
                <div class="result-title">▸ {r['shape']} 유도 공식</div>
            </div>""", unsafe_allow_html=True)
            st.latex(f"A = {r['A_expr']}")
            st.latex(f"h_c = {r['hc_expr']}")
            st.latex(f"F = {r['F_expr']}")
            ...
            <div class="result-card" style="margin-top:.45rem">
                ...
                <div style="font-family:'Share Tech Mono',monospace;font-size:.74rem;
                            color:#c0d8e8;line-height:2.1">
                    A = {r['A']:.4f} m²<br>
        ...
        st.markdown("""<div class="info-box" style="text-align:center;padding:3rem">
            ...
            <div style="font-family:'Rajdhani',sans-serif;font-size:1.05rem;color:#00c8ff;
                        margin-bottom:.35rem;letter-spacing:.1em">시각화 대기 중</div>
...
""")
    ...
    st.markdown("""<div class="info-box" style="display:flex;gap:2rem;flex-wrap:wrap;
        justify-content:center;padding:.7rem 1.4rem;margin-top:.8rem">
        ...
        <span><b style="color:#00c8ff">Claude API</b> — LLM</span>
