"""
llm_parser.py  v3.5
다중 LLM API + 로컬 해설 생성 fallback + 강화된 규칙 파서
핵심: API 실패 시에도 전문적인 해설을 로컬에서 자동 생성
"""
...
# API Providers (import 실패해도 동작하도록)
try:
    import anthropic
except ImportError:
    anthropic = None
...
응답 JSON 형식만:
{"shape":"rectangle","params":{"width":3.0,"height":5.0,"h_w":1.0},"confidence":0.95,"interpretation":"설명"}
...
CHAT_SYSTEM = """당신은 '하이드로스텟 AI'라는 댐 수문 설계 전문 AI 어시스턴트입니다.
한국어로 전문적이고 친절하게 응답하세요. 수식은 LaTeX 포맷 사용.
"""
...
def extract_params(user_text: str, provider: str, api_key: str) -> Tuple[Optional[Dict], str]:
    try:
        raw = _call_llm(provider, api_key, EXTRACTION_SYSTEM, user_text, max_retries=1)
        json_match = re.search(r'\{[\s\S]*\}', raw)
        if not json_match:
            return None, f"JSON 추출 실패"
...
def explain_result(result: Dict, user_query: str,
    ...
    """
    계산 결과 해설: API 시도 → 실패 시 로컬 해설 자동 생성
    """
    # 1) API 시도
    try:
        summary = (
...
def _local_explain(r: Dict) -> str:
    ...
    e = ycp - hc
    IG = r.get('IG', 0)
    M = r.get('M_overturn', F_N * ycp) / 1000  # kN·m
    params = r.get('params', {})
    hw = params.get('h_w', 0)
